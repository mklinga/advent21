...
async function process() {
    ...
    function findLastBingo() {
        const bingoes = tables.map(() => false);
        for (let bindex = 0; bindex < bingoNumbers.length; bindex++) {
            ...
            for (let tindex = 0; tindex < tables.length; tindex++) {
                ...
                if (table.hasBingo()) {
                    if (!bingoes[tindex]) {
                        console.log('BINGO! for table', tindex);
                    }
                
                    bingoes[tindex] = true;
                    if (bingoes.reduce((acc, b) => acc && b)) {
                        // All the tables have now bingo, return last tindex and winning number
    ...
    const [winner, winningNumber] = findLastBingo();
